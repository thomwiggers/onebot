...
def _get_patched_time_fixture(fixture_name, **kwargs):
    ...
    if not type(fixture["track"]) == list:
        ...
        fixture["track"][0]["date"]["uts"] = str(calendar.timegm(date.timetuple()))
...
class LastfmPluginTest(BotTestCase):
    ...
    config = {
        ...
        "onebot.plugins.lastfm": {"api_key": "", "api_secret": ""},
    ...
    def test_get_lastfm_nick_from_database(self):
        ...
        # mock get_setting
        async def mock_get_setting(setting, default):
    ...
    @patch("lastfm.lfm.Track.get_info", side_effect=lastfm.exceptions.InvalidParameters)
    ...
    @patch(
        "lastfm.lfm.Track.get_info",
        return_value=_get_fixture("track_get_info_m83_graveyard_girl.json"),
    )
    ...
    @patch(
        "lastfm.lfm.User.get_recent_tracks",
        return_value=_get_fixture("user_get_recent_tracks_loved_now_playing.json"),
    )
    @patch("lastfm.lfm.Track.get_info", side_effect=lastfm.exceptions.InvalidParameters)
    ...
    @patch(
        ...
        return_value=_get_fixture("user_get_mbid_tags_M83.json"),
    )
